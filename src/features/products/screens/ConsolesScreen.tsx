...
export const ConsolesScreen = () => {
  ...
  const consoles = productsList.filter((p: Product) => 
  ...
  return (
    <ScreenWrapper padding={false} scrollable={false} className="bg-black">
      <View className="px-6 py-6 border-b border-white/5 bg-surface-container-high/40">
        <Text className="text-white text-3xl font-headline font-bold">Consoles</Text>
        <Text className="text-[#adaaaa] text-xs uppercase tracking-widest mt-1">Premium Gaming Hardware</Text>
      </View>
      ...
      <FlatList
        ...
        renderItem={({ item }) => (
          ...
          >
            ...
            <Text className="text-primary font-bold mx-1">${item.price}</Text>
          </TouchableOpacity>
        )}
        ListEmptyComponent={
          <View className="flex-1 justify-center items-center py-20">
            <Text className="text-[#adaaaa]">No consoles currently available.</Text>
          </View>
        }
      />
    </ScreenWrapper>
