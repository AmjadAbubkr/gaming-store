...
export const GamesScreen = () => {
  ...
  useEffect(() => {
    ...
    const loadGames = async () => {
      try {
        const products = await firestoreService.getProductsByCategories(GAME_CATEGORIES, 12);
        if (isActive) {
          setGames(products);
        }
      } catch {
  ...
  return (
    ...
    >
      <View className="pt-0">
        <ImageBackground
          source={require('../../../../assets/games-hero.png')}
      ...
      <View className="border-b border-white/5 bg-surface-container-high/40 px-6 py-6">
        <Text className="text-3xl font-headline font-bold text-white">{t('catalog.game')}</Text>
        <Text className="mt-1 text-xs uppercase tracking-widest text-[#adaaaa]">
          {t('catalog.gameSubtitle')}
        </Text>
      </View>
      ...
      {isLoading ? (
        ...
        <FlatList
          ...
          contentContainerStyle={{ padding: 16, flexGrow: games.length === 0 ? 1 : undefined }}
          ...
          renderItem={({ item }) => (
            ...
            >
              ...
              <Text className="mx-1 font-bold text-secondary">{formatPrice(item.price)}</Text>
            </TouchableOpacity>
          )}
        />
      )}
